...
export default class ParameterInjector {
    ...
    static inject(target, params) {
        const instance = (new ParameterInjector());
        ...
        if (_.isPlainObject(target) || _.isArray(target)) {
            return instance.injectIntoObject(target, _.clone(params));
        }
    ...
    injectIntoObject(obj, params) {
        const preparedString = this.injectionPreparation(obj);
        const newString = this.injectParams(preparedString, params);
        const newObject = this.injectionPreparation(newString);
    ...
    injectionPreparation(obj) {
        if (_.isString(obj) && obj.indexOf(this.delimiter) !== -1) {
            const objArray = obj.split(this.delimiter);
