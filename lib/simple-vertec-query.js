...
export class SimpleVertecQuery {
    ...
    setCacheGraceTime(value) {
        this.options.cacheGraceTime = value;
    ...
    get(refresh = false) {
        if (this.options.useParallelMode && this.options.query.objref) {
            let ids = _.isArray(this.options.query.objref) ? _.cloneDeep(this.options.query.objref) : [this.options.query.objref];

            return q.map(ids, id => {
                let newOptions = _.cloneDeep(this.options);
                return new SimpleVertecQuery(newOptions).inParallel(false).findById(id).get(refresh);
            });
        }
