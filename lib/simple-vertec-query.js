 ...
 */
export default class SimpleVertecQuery {
    /**
    ...
    /**
     * Sets global cache instance for every instance
     *
     * @param {object} cache A cache instance with get(key) and set(key, value, ttl) methods
     *
     * @return {void}
     */
     ...
     *
     * @param {number} id One id
     *
     * @return {object} SimpleVertecQuery
     */
    findById(id) {
        this.options.query.objref = id;
     ...
     */
    addFields(...args) {
        const items = _.isArray(args[0]) ? args[0] : args;

        this.options.fields.push(...items);
    ...
    /**
     * Sets optional cache name for that cache entry
     *
     * Will be used for cache key generation
     *
     * @param {string} value Item cache key
     *
     * @return {object} SimpleVertecQuery
    ...
    /**
     * Routes this query through the slow lane pool for heavy/deprioritized requests
     *
     * @param {boolean} slowLane Enable or disable slow lane (default: true)
     *
     * @return {object} SimpleVertecQuery
     */
    usingSlowLane(slowLane = true) {
        this.options.slowLane = slowLane;
                ...
                return SimpleVertecQuery
                    ...
                    .then(cacheData => {
                        ...
                        this.makeRequest().then(result => {
                            // to maintain backwards compatibility, we take seconds and convert them to milliseconds
                            let cacheDuration = (this.options.cacheTTL + this.options.cacheGraceTime) * 1000;

                            let meta = {
                                cacheDateTime: currentTime,
                                softExpire: currentTime + this.options.cacheTTL,
                                onGrace: false
                            };
                            ...
                            SimpleVertecQuery
                                ...
                                .then(() => {
                                    cacheData.meta.refresh = refresh;

                                    resolve(cacheData);
                                })
                                .catch(err => {
                                    if (isGraceRefresh) {
                                        console.error('[SimpleVertecQuery] Grace period cache set error:', err);
                                    } else {
                                        reject(err);
                                    }
                                });
                            ...
                            if (isGraceRefresh) {
                                console.error('[SimpleVertecQuery] Grace period refresh error:', err);
                            } else {
                                reject(err);
                            }
                        });
    ...
    getCacheKey() {
        let cacheKey = [
            ...
            (this.options.cacheKey ? this.options.cacheKey : md5(SimpleVertecQuery.api.buildXml({query: this.options.query, params: this.options.params, fields: this.options.fields}))),
     ...
     */
    makeRequest() {
        let args = [this.options.query, this.options.params, this.options.fields];
        if (this.options.slowLane) {
            args.push({ slowLane: true });
        }
        let request = SimpleVertecQuery.api.select(...args);

        if (!this._filterPropertiesTransformerAdded) {
            this.addFilterPropertiesTransformer();
            this._filterPropertiesTransformerAdded = true;
        }
