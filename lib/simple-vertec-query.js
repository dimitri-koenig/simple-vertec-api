...
export class SimpleVertecQuery {
     ...
     *
     * @param {number} value Seconds for item to be in cache
     *
     * @return {object} SimpleVertecQuery
     */
    setCacheTTL(value) {
     ...
     *
     * @param {number} value Seconds for item to be additionally in cache
     *
     * @return {object} SimpleVertecQuery
     */
    ...
    filterProperty(key, toArray = false) {
        this.propertyFilter = {key, toArray};
     ...
     *
     * @param {path} path Path to the object property
     *
     * @return {object} SimpleVertecQuery
     */
    zip(path) {
        this.addZipTransformer(path);

        return this;
    }

    /**
     * Activates parallel fetching mode of multiple objrefs
     *
     * @return {object} SimpleVertecQuery
     */
    inParallel() {
     ...
     */
    get(refresh = false) {
        if (this.useParallelMode && _.isArray(this.query.objref) && this.query.objref.length > 1) {
            let ids = _.clone(this.query.objref);

            return q.map(ids, id => {
                return _.clone(this).findById(id).get();
            });
        }
    ...
    addFilterPropertiesTransformer() {
        if (this.propertyFilter) {
            this.transformer.unshift(response => {
                let newResponse = response[this.propertyFilter.key];

                if (this.propertyFilter.toArray && !_.isArray(newResponse)) {
     ...
     * @return {void}
     */
    addZipTransformer(path) {
        this.transformer.push(response => {
