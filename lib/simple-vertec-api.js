...
export class SimpleVertecApi {
    constructor(xmlUrl, authUrl, username, password, verbose, defaultRequestOptions) { // eslint-disable-line max-params
        ...
        // workaround: didn't found a way of checking for auth token validity, so reset auth token every hour
        setInterval(this.resetAuthToken, 60 * 60 * 1000);
    }
    ...
    requestRetryStrategy(err, response) {
        let check = (err !== null) || !_.isObject(response) || (response.statusCode >= 400) || (_.isString(response.body) && (/<html>/i.test(response.body) || /<fault>/i.test(response.body) || /Internal Server Error/i.test(response.body)));
