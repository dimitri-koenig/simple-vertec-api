...
export default class SimpleVertecApi {
    ...
    doStoredRequest(xmlString) {
        ...
        return this.storedPromises[hash] = this.enqueueRequest(xmlString);
    }
     ...
     * @return {Promise} Promise for the request
     */
    enqueueRequest(xmlString) {
        return new q((resolve, reject) => {
            this.requestQueue.push({ xmlString, resolve, reject });
            this.processQueue();
        });
    }
    ...
    request(xmlString) {
        ...
        if (this.defaultRequestOptions['fixedSessionTag'] != null) {
            vertecSessionForRequest = this.defaultRequestOptions['fixedSessionTag'];
        } else {
