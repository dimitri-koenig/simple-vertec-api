...
export class SimpleVertecApi {
    ...
    /**
     * Creates one or many new objects
     *
     * @return {Promise} Promise for the request
     */
    create() {
        ...
        if (newObjects.length === 0) {
            throw new Error('[1439115447] No valid create object data found');
        }

        var createObject = this.buildCreateBody(newObjects);
        var xmlString = this.buildXml(createObject);
     ...
     * @private
     *
     * @param {array} newObjects An array of new objects to create
     *
     * @return {object} Body part of the XML request
     */
    buildCreateBody(newObjects) {
        var content = {
            Body: [
                {
                    Create: []
        ...
        _.each(newObjects, (newObject) => {
            if (!_.isString(newObject.className) || !_.isPlainObject(newObject.data)) {
                throw new Error('[1439114369] No valid create object data found');
            }

            var tempNew = {};
            tempNew[newObject.className] = this.convertObjectToXmlFriendlyFormat(newObject.data);
            content.Body[0].Create.push(tempNew);
        });
    ...
    convertObjectToXmlFriendlyFormat(data) {
        ...
        _.each(data, (value, key) => {
            ...
            if (_.isPlainObject(value)) {
                value = this.convertObjectToXmlFriendlyFormat(value);
            }
            temp[key] = value;
            content.push(temp);
        });
