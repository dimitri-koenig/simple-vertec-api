...
export class SimpleVertecApi {
    ...
    escapeValue(value) {
        return !_.isString(value) ? value : value.replace(/[<>&'"]/g, (char) => {
            switch (char) {
                case '<': return '&lt;'; // eslint-disable-line indent
                case '>': return '&gt;'; // eslint-disable-line indent
                case '&': return '&amp;'; // eslint-disable-line indent
                case '\'': return '&apos;'; // eslint-disable-line indent
                case '"': return '&apos;'; // eslint-disable-line indent
    ...
    buildSelectBody(select, fields) {
        ...
        _.each(contentObject.Body.Query.Selection, (value, key) => {
            contentObject.Body.Query.Selection[key] = this.escapeValue(value);
        });
