...
export class SimpleVertecApi {
    ...
    requestRetryStrategy(err, response) {
        let check = (err !== null) || (response.statusCode >= 400) || (_.isString(response.body) && (/<html>/i.test(response.body) || /<fault>/i.test(response.body)));
     ...
     *
     * First parameter is either a query or an array
     * of arguments usable for this method
    ...
    select() {
        ...
        if (_.isUndefined(query) || ((!_.isString(query) && !_.isPlainObject(query) && !_.isArray(query)))) {
        ...
        if (_.isArray(query)) {
            ...
            _.each(query, singleQuery => {
                let singleSelectObject = this.buildSelectObject(...singleQuery);
                selectObject.Query.push(singleSelectObject.Query);
            });
        } else {
            selectObject = this.buildSelectObject(...arguments);
        }
    ...
    transformDotKeys(obj) {
        traverse(obj).forEach(function(node) {
            if (this.key && _.size(this.key.match(/\./g)) === 1) {
