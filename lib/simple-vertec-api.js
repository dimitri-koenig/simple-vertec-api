...
export class SimpleVertecApi {
    ...
    convertParam(param) {
        if (param instanceof Date || moment.isMoment(param)) {
            console.log('Deprecated: conversion of a Date/Moment object will be removed in 2.5'); // eslint-disable-line no-console
    ...
    buildSelectBody(select, fields) {
        ...
        if (_.isPlainObject(select)) {
            contentObject.Body.Query.Selection = select;
        } else {
            contentObject.Body.Query.Selection.ocl = select;
        }
    ...
    buildXml(bodyObject) {
        ...
        var xmlStringParts = [
            ...
            XmlConverter.toXml(contentObject)
        ];
    ...
    buildDeleteBody(ids) {
        ...
        _.each(ids, (id) => {
            ...
            content.Body.Delete.objref.push(parseInt(id));
        });
    ...
    buildSaveBody(saveObjects) {
        ...
        if (_.size(createObjects) > 0) {
            _.extend(content.Body, {
                Create: createObjects
            });
        }

        if (_.size(updateObjects) > 0) {
            _.extend(content.Body, {
    ...
    convertFieldOptions(fields) {
        ...
        _.forEach(fields, (field) => {
            if (_.isString(field)) {
                return members.push(field);
            }

            if (_.isObject(field)) {
                return expressions.push(field);
            }
        ...
        if (members.length > 0) {
            fieldOptions.member = members;
        }

        if (expressions.length > 0) {
            fieldOptions.expression = expressions;
        }
